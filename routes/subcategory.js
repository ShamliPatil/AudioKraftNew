...
router.delete('/deleteSubCategoryBySubCategoryId', auth, async (req, res) => {
  subcategoryId = req.query.subcategoryId;
  if(!mongoose.Types.ObjectId.isValid(subcategoryId)) return res.status(400).send({statusCode:400,error:'Bad Request',message:'Please provide valid subcategoryId.'});
  let subcategory =  await SubCategory.findByIdAndDelete({ _id :subcategoryId });
  if(!subcategory) return res.status(404).send({ statusCode : 404, error : 'Not Found' , message : 'Subcategory not found.' }); //Not Found
  return res.status(200).send({statusCode : 200,message : 'Subcategory Successfuly delete.' });
